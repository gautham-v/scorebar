...
A tag is what publishes. `.github/workflows/release.yml` then builds `scorebar` for
...
Pushes to `main` run the same build and publish nothing. They are there for the cache: GitHub
scopes caches by ref, so a cache written by one tag's run is invisible to the next tag's, while
every ref can read `main`'s.
